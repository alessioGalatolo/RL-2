...
# Code authors: [Alfred Nilsson - alfredn@kth.se][Alessio Galatolo - galatolo@kth.se]

# The save file of the neural network needs to be named 'neural-network-1.pth'
...
def main():
    ...
    C_target = int(config['replay_buffer_size'] / config['batch_size_train'])
    ...
    agent = CleverAgent(n_actions, dim_state, device, config['eps_max'],
                        config['eps_min'], decay_period=decay_period,
    ...
    q_network = Model(d_in=dim_state,
                      hidden_layers=hidden_layers,
                      d_out=n_actions).to(device)
    ...
    EPISODES = tqdm(range(start_episode, config['N_episodes'], 1),
                    desc='Episode: ', leave=True,
                    initial=start_episode, total=config['N_episodes'])
    ...
    for i in EPISODES:
        ...
        while not done:
            ...
            with torch.no_grad():
                target_qvals = agent.get_qvals(next_state_train, target_network).max(1).values
                target_val = reward_train + config['discount_factor'] * target_qvals * (1 - done_train)
        ...
        if wandb is not None:
            wandb.log({'loss': detached_loss,
                       'total_episode_reward': total_episode_reward,
                       'reward_running_avg': reward_running_avg,
                       'episode': i}, step=i)
